...
var _ = server.Add(func(app *gin.Engine) {
	app.POST("/application/remove", func(ctx *gin.Context) {
		...
		go func() {
			...
			responses2.ChannelSend(channel, responses2.CreateChannelOk("Removing main service container (burp."+burp.Service.Name+")...."))
			if err := docker.Remove(fmt.Sprint("burp.", burp.Service.Name)); err != nil {
				...
				responses2.ChannelSend(channel, responses2.CreateChannelError("Failed to remove service container (burp."+burp.Service.Name+")", err.Error()))
				return
			}
			responses2.ChannelSend(channel, responses2.CreateChannelOk("Removed main service container (burp."+burp.Service.Name+")"))
			for _, dependency := range burp.Dependencies {
				responses2.ChannelSend(channel, responses2.CreateChannelOk("Removing dependency container (burp."+dependency.Name+")...."))
				if err := docker.Remove(fmt.Sprint("burp.", dependency.Name)); err != nil {
					...
					responses2.ChannelSend(channel, responses2.CreateChannelError("Failed to remove dependency container (burp."+dependency.Name+")", err.Error()))
					return
				}
				responses2.ChannelSend(channel, responses2.CreateChannelOk("Removed dependency container (burp."+dependency.Name+")"))
			}
			defer close(*channel)
		}()

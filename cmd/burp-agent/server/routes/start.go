	...
	app.POST("/application/start", func(ctx *gin.Context) {
		...
		go func() {
			...
			for _, dependency := range burp.Dependencies {
				responses2.ChannelSend(channel, responses2.CreateChannelOk("Starting dependency container (burp."+dependency.Name+")...."))
				if err := docker.Start(fmt.Sprint("burp.", dependency.Name)); err != nil {
					...
					responses2.ChannelSend(channel, responses2.CreateChannelError("Failed to start dependency container (burp."+dependency.Name+")", err.Error()))
					return
				}
				responses2.ChannelSend(channel, responses2.CreateChannelOk("Started dependency container (burp."+dependency.Name+")"))
			}

			responses2.ChannelSend(channel, responses2.CreateChannelOk("Starting main service container (burp."+burp.Service.Name+")...."))
			if err := docker.Start(fmt.Sprint("burp.", burp.Service.Name)); err != nil {
				...
				responses2.ChannelSend(channel, responses2.CreateChannelError("Failed to start main service container (burp."+burp.Service.Name+")", err.Error()))
				return
			}
			responses2.ChannelSend(channel, responses2.CreateChannelOk("Started main service container (burp."+burp.Service.Name+")"))
			defer close(*channel)
		}()
